...
async function onBatch(body, settings) {
  const { idSchema: id_schema, audienceId, advertiserIds } = settings;
  ...
  for (const user of body) {
    if (user.type == "identify") {
      ...
      //   user.traits.email = "";
      // }
      user.traits.email ?? ""; // can const email as well
      //PEER REVIEW NEED SEPARATE DESTINATION INSTANCES WHEN ACTIVATING?
      //HOW DO SETTING VARIABLES WORK FOR DIFFERENT AUDIENCES ADD TO SETUP
  ...
  if (addMappingBody.batch_data.length > 0)
    await tiktokHttpRequest(addMappingBody, settings);
  if (deleteMappingBody.batch_data.length > 0)
    await tiktokHttpRequest(deleteMappingBody, settings);
}
...
async function getPhoneNumber(externalId, settings) {
  ...
  if (profile_api_response.ok) {
    ...
    //extract all phone / mobile numbers from traits object (PEER REVIEW NOTES change to settings variable)
    const obj = traits;
    ...
    //collect last updated phone number // PEER REVIEW NOTES SORT IN API
